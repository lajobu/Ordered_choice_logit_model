...
survey <- read.csv("survey_results_public_2019.csv") # for 2019 instead of 2018
source("ome.R")
```
...
```{r echo= TRUE, results='asis'}
colnames(survey) <- c('CareerSatisfaction', 'Country', 'FormalEducation', 
...
survey$CompanySize <- plyr::revalue(
  survey$CompanySize,
  c(
    "2-9 employees" = 1, # changed from "Fewer than 10 employees" to "2-9 employees"
...
unique(survey$YearsCoding) %>% 
  knitr::knit_print() # format change, intead of groups, there are just values
```

```{r echo= TRUE, results='hide'}
survey$YearsCoding[survey$YearsCoding == "Less than 1 year"] <- 1
survey$YearsCoding[survey$YearsCoding %in% (3:5)] <- 1

survey$YearsCoding[survey$YearsCoding %in% (6:17)] <- 2

survey$YearsCoding[survey$YearsCoding %in% (18:50)] <- 3 
```
...
```{r echo= TRUE, results='hide'}
survey$JobSatisfaction <- # same change as CareerSatisfaction
  plyr::revalue(
...
survey$Age[survey$Age %in% (0:24)] <- 1

survey$Age[survey$Age %in% (25:44)] <- 2

survey$Age[survey$Age %in% (45:100)] <- 3 
